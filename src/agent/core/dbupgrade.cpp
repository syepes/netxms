...
static BOOL H_UpgradeFromV6(int currVersion, int newVersion)
{
   ...
   if (hResult != NULL)
   {
      ...
      DB_STATEMENT hStmt = DBPrepare(s_db, _T("UPDATE agent_policy SET guid=?,type=?"));
      if(hStmt != NULL)
      {
         for(int row = 0; row < DBGetNumRows(hResult); row++)
         {
            DBBind(hStmt, 1, DB_SQLTYPE_VARCHAR, DBGetFieldGUID(hResult, row, 0));
            DBBind(hStmt, 2, DB_SQLTYPE_VARCHAR, policyNames[DBGetFieldULong(hResult, row, 1)], DB_BIND_STATIC);

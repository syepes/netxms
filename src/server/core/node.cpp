...
void Node::checkAgentPolicyBinding(AgentConnection *conn)
{
   ...
   if (rcc == ERR_SUCCESS)
   {
      ...
      for(int i = 0; i < m_parentList->size(); i++)
      {
         if (m_parentList->get(i)->getObjectClass() == OBJECT_TEMPLATE)
         {
            ObjectArray<GenericAgentPolicy> *policyList = ((Template*)m_parentList->get(i))->getpolicyListCopy();
            for(int n = 0; n < policyList->size(); n++)
            {
               const uuid& guid = policyList->get(n)->getGuid();
               int j;
               for(j = 0; j < ap->size(); j++)
               {
               ...
               if (j == ap->size())
               {
                  ServerJob *job = new PolicyInstallJob(this, m_parentList->get(i)->getId(), guid, policyList->get(n)->getName(), 0);
                  if (AddJob(job))
                  {

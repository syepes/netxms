...
GenericAgentPolicy::GenericAgentPolicy(GenericAgentPolicy *policy)
{
   ...
   m_fileContent = _tcsdup(policy->m_fileContent);
}
...
bool GenericAgentPolicy::saveToDatabase(DB_HANDLE hdb)
{
   ...
   if (!IsDatabaseRecordExist(hdb, _T("ap_common"), _T("guid"), m_guid)) //Policy can be only created. Policy type can't be changed.
      hStmt = DBPrepare(hdb, _T("INSERT INTO ap_common (name,owner_id,policy_type,file_content,guid) VALUES (?,?,?,?)"));
   else
      hStmt = DBPrepare(hdb, _T("UPDATE ap_common SET name=?,owner_id=?,policy_type=?,file_content=? WHERE guid=?"));
...
bool GenericAgentPolicy::loadFromDatabase(DB_HANDLE hdb)
{
   ...
   _sntprintf(query, 256, _T("SELECT name,owner_id,policy_type,file_content WHERE guid=%s"), (const TCHAR *)m_guid.toString());
   ...
   if (hResult != NULL && DBGetNumRows(hResult) > 0)
   {
      ...
      m_fileContent = DBGetField(hResult, 0, 2, NULL, 0);
 ...
 */
void GenericAgentPolicy::fillMessageInternal(NXCPMessage *msg, UINT32 baseId)
{
   msg->setField(baseId, m_guid);
   msg->setField(baseId+1, m_policyType);
   msg->setField(baseId+2, CHECK_NULL_EX(m_name));
 ...
 */
UINT32 GenericAgentPolicy::modifyFromMessageInternal(NXCPMessage *msg, UINT32 baseId)
{
   msg->getFieldAsString(baseId+2, m_name, MAX_DB_STRING);

   if (msg->isFieldExist(baseId+3))
   {
      MemFree(m_fileContent);
      m_fileContent = msg->getFieldAsString(baseId+3);
   }
   return RCC_SUCCESS;
}

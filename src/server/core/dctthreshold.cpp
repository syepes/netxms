...
{
   DB_STATEMENT hStmt = DBPrepare(hdb, _T("SELECT instance,match_count,is_active,row FROM dct_threshold_instances WHERE threshold_id=? AND bmaint_copy='0'"));
   if (hStmt == NULL)
      return;
   ...
   hStmt = DBPrepare(hdb, _T("SELECT instance,match_count,is_active,row FROM dct_threshold_instances WHERE threshold_id=? AND bmaint_copy='1'"));
   if (hStmt == NULL)
      return;
...
bool DCTableThreshold::saveToDatabase(DB_HANDLE hdb, UINT32 tableId, int seq)
{
   ...
   {
      hStmt = DBPrepare(hdb, _T("INSERT INTO dct_threshold_instances (threshold_id,instance,match_count,is_active,row,bmaint_copy) VALUES (?,?,?,?,?,?)"));
      if (hStmt == NULL)
         return false;

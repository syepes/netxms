...
static int F_GetDCIValues(int argc, NXSL_Value **argv, NXSL_Value **ppResult, NXSL_VM *vm)
{
	...
	if ((dci != NULL) && (dci->getType() == DCO_TYPE_ITEM))
	{
      ...
      {
        _sntprintf(query, 1024, _T("SELECT idata_value FROM idata WHERE node_id=%u AND item_id=? AND idata_timestamp BETWEEN ? AND ? ORDER BY idata_timestamp DESC"), node->getId());
      }
      else
      {

 ...
 *        repeat_interval,current_severity,last_event_timestamp,match_count,before_maint_state FROM thresholds
 */
   ...
   // Prepare and execute query
	...
	if (!IsDatabaseRecordExist(hdb, _T("thresholds"), _T("threshold_id"), m_id))
	{
		hStmt = DBPrepare(hdb,
			...
			_T("sequence_number,current_state,before_maint_state,rearm_event_code,repeat_interval,")
	...
	else
	{
		hStmt = DBPrepare(hdb,
         ...
         _T("sequence_number=?,current_state=?,before_maint_state=?,rearm_event_code=?,")

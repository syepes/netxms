...
int main(int argc, char *argv[])
{
   ...
   InitNetXMSProcess();
...
#else
			case 'p':
				strncpy(s_pidFile, optarg, MAX_PATH);
				s_pidFile[MAX_PATH - 1] = 0;
				break;
#endif
			case '?':
				return 1;
	...
	{
		if (_taccess(PREFIX _T("/etc/nxflowd.conf"), 4) == 0)
		{
			_tcscpy(g_configFile, PREFIX _T("/etc/nxflowd.conf"));
		}
		else if (_taccess(_T("/usr/etc/nxflowd.conf"), 4) == 0)
		{
			_tcscpy(g_configFile, _T("/usr/etc/nxflowd.conf"));
		}
		else
		{
			_tcscpy(g_configFile, _T("/etc/nxflowd.conf"));
		}
	}
#endif
